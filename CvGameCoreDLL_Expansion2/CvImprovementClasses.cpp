...
bool CvImprovementEntry::CacheResults(Database::Results& kResults, CvDatabaseUtility& kUtility)
{
	...
	//YieldPerXAdjacentImprovement
	{
		...
		if (pResults == NULL)
		{
			pResults = kUtility.PrepareResults(strKey, "select Yields.ID as YieldID, Improvements.ID as ImprovementID, Yield, NumRequired from Improvement_YieldPerAdjacentImprovement inner join Yields on YieldType = Yields.Type inner join Improvements on OtherImprovementType = Improvements.Type where ImprovementType = ?");
		}
		...
		while (pResults->Step())
		{
			...
			m_YieldPerXAdjacentImprovement[yield_idx][improvement_idx] = std::make_pair(yield, nRequired);
		}

		//Trim extra memory off container since this is mostly read-only.
		map<YieldTypes, map<ImprovementTypes, pair<int, int>>>(m_YieldPerXAdjacentImprovement).swap(m_YieldPerXAdjacentImprovement);
	}
...
pair<int, int> CvImprovementEntry::GetYieldPerXAdjacentImprovement(YieldTypes eYield, ImprovementTypes eImprovement) const
{
	...
	pair<int, int> fYield = make_pair(0,1);
	...
	{
		map<ImprovementTypes, pair<int, int>>::const_iterator itYield = itImprovement->second.find(eImprovement);
	...
	{
		fYield.first += m_iCultureAdjacentSameType * fYield.second;
	}
