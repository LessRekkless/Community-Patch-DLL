...
-- Aesthetics Change
UPDATE Language_en_US
SET Text = 'When improved with an Archaeological Dig, a Hidden Antiquity Site can then yield an Artifact that can be placed in a [ICON_GREAT_WORK] Great Work of Art slot, or be transformed into a Landmark improvement. Hidden Antiquity Sites are only visible to Civilizations that have all the policies in the Artistry policy tree.'
...
UPDATE Language_en_US
SET Text = 'Players interested in creating massive, sprawling Civilizations should turn to the Order ideology, as the strength of the empire is determined by the total number of cities it contains. [COLOR_YELLOW]This ideology unlocks upon entering the Atomic Era, or unlocking 18 Policies and have advanced at least to the Industrial Era, whichever comes first.[ENDCOLOR]'
